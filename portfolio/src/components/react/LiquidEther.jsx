  ...
  useEffect(() => {
    ...
    class MouseClass {
      constructor() {
        ...
        this.coords_target = new THREE.Vector2();
      ...
      setCoords(x, y) {
        ...
        this.coords_target.set(nx * 2 - 1, -(ny * 2 - 1));
      ...
      update() {
        if (this.takeoverActive) {
          ...
          if (t >= 1) {
            ...
            this.coords_target.copy(this.takeoverTo);
            this.coords_old.copy(this.coords);
            this.diff.set(0, 0);
          } else {
            const k = t * t * (3 - 2 * t);
            this.coords.copy(this.takeoverFrom).lerp(this.takeoverTo, k);
            this.coords_target.copy(this.coords);
          }
        } else if (!this.isAutoActive) {
          const delay = Math.min(100, Math.max(0, this.mouseDelay));
          const follow = 1 - delay / 100;
          const lerpAmount = 0.04 + follow * 0.96;
          this.coords.lerp(this.coords_target, lerpAmount);
        }
    ...
    class AutoDriver {
      constructor(mouse, manager, opts) {
        ...
        this.active = false;
        this.current = new THREE.Vector2(-0.82, 0.18);
        this.target = new THREE.Vector2(0.72, 0.04);
...
`;
    ...
    void main(){
    vec2 vel = texture2D(velocity, uv).xy;
    float lenv = clamp(length(vel) * colorBoost, 0.0, 1.0);
    float gradientPos = clamp(uv.x * 0.62 + (1.0 - uv.y) * 0.38 + vel.x * 0.22 - vel.y * 0.14, 0.0, 1.0);
    vec3 c = texture2D(palette, vec2(gradientPos, 0.5)).rgb;
  ...
  useEffect(() => {
    ...
    if (webgl.autoDriver) {
      ...
      if (webgl.autoDriver.mouse) {
        ...
        webgl.autoDriver.mouse.mouseDelay = mouseDelay;
      }
    }
    if (resolution !== prevRes) {
      sim.resize();
